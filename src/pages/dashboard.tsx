...
import { AdminListPage as ShowCaseList } from 'pages/admins';
...
function DashboardPage() {
  ...
  const adminStat = useSWR<PaginatedInterface<AdminInterface>>(
    () => `/admins`,
    () => getAdmins({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Admins',
      result: adminStat?.data?.totalCount,
      isLoading: adminStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`admins`} passHref legacyBehavior>
                  <Button
